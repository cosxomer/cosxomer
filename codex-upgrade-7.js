(() => {
    ...
    async function syncRealtimeTimer(reason = "manual", options = {}) {
        ...
        if (getFreshForeignActiveTimer()) {
            syncCurrentUserLiveDoc(currentUserLiveDoc || {}, { silent: reason === "interval" });
            return;
        }
    ...
    function restoreTimerFromPersistence(userData = {}) {
        ...
        if (stored && String(stored.ownerId || "").trim() && stored.ownerId !== timerInstanceId) {
            const storedElapsed = getTimerElapsedSeconds(stored, Date.now());
            stored = {
                ...stored,
                isRunning: false,
                startedAtMs: 0,
                baseElapsedSeconds: storedElapsed,
                lastPersistedElapsedSeconds: storedElapsed
            };
        }
        ...
        if (getFreshForeignActiveTimer(userData)) {
            timerState.session.isRunning = false;
            timerState.session.startedAtMs = 0;
        }
