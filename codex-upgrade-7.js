(() => {
    ...
    let activeMoveSelection = null;
    let moveTaskSelections = {};
...
const BROKEN_UI_TEXT_REPLACEMENTS = [
    ...
    function syncCurrentUserLiveDoc(userData = {}, options = {}) {
        ...
        const resolvedUsername = pickPreferredProfileText(
            [
                ...
                currentUserPublicProfileDoc?.username,
                currentUserPublicProfileDoc?.name,
                currentUsername
            ],
            { email: resolvedEmail }
        );
    ...
    function isLegacyDefaultProfileIdentityValue(value = "") {
        const normalizedValue = String(value || "")
            .trim()
            .toLocaleLowerCase("tr-TR")
            .replace(/\s+/g, "");
        return normalizedValue === "omer" || normalizedValue === "ömer";
    }
    ...
    async function resetRealtimeTimer(resetInputs = true, silent = false) {
        ...
        if (timerState.session) {
            const commitSourceSession = createCommitSourceSession(timerState.session);
            const commitState = commitTimerSessionLocally(commitSourceSession, {
                referenceMs: Date.now(),
                persistRecovery: true
            });
            committedDelta = Math.max(0, parseInteger(commitState.committedDeltaSeconds, 0));
            timerState.session.lastPersistedElapsedSeconds = Math.max(
                parseInteger(timerState.session.lastPersistedElapsedSeconds, 0),
                parseInteger(commitState.committedElapsedSeconds, 0)
            );
        }
        ...
        if (!silent) {
            safeShowAlert(
                committedDelta > 0
                    ? "Sayaç sıfırlandı. Çalıştığın süre korunarak kaydedildi."
                    : "Sayaç sıfırlandı. Ana süre korunuyor.",
                "success"
            );
        }
    }
    ...
    function patchProfileSaveFlow() {
        saveProfileChanges = async function() {
            ...
            try {
                ...
                if (latestPayload && currentUser?.uid) {
                    await withManualFirestoreWrite(async () => {
                        await db.collection("users").doc(currentUser.uid).set(latestPayload, { merge: true });
                        if (typeof syncPublicProfileSnapshot === "function") {
                            await syncPublicProfileSnapshot(latestPayload);
                        }
                    });
                } else if (typeof syncPublicProfileSnapshotSafely === "function") {
                    await syncPublicProfileSnapshotSafely(latestPayload);
                }
    ...
    function ensureMoveTaskStyles() {
        if (document.getElementById("codex-move-task-styles")) return;
        const style = document.createElement("style");
        style.id = "codex-move-task-styles";
        style.textContent = `
            .move-task-checkbox {
                width: 16px;
                height: 16px;
                margin-right: 8px;
                accent-color: var(--countdown-fill);
                flex-shrink: 0;
                cursor: pointer;
            }
        `;
    ...
    function enhanceMoveTaskUI() {
        ...
        for (let dayIdx = 0; dayIdx < 7; dayIdx++) {
            const buttonGroup = document.querySelector(`#task-list-${dayIdx}`)
                ? document.getElementById(`q-input-${dayIdx}`)?.closest(".question-input-box")?.querySelector(".question-btn-group")
                : null;

            if (buttonGroup) {
                const isMoveMode = activeMoveSelection
                    && activeMoveSelection.weekKey === currentWeekKey
                    && activeMoveSelection.dayIdx === dayIdx;

                const existingMoveBtn = buttonGroup.querySelector(".move-task-btn");
                const existingCancelBtn = buttonGroup.querySelector(".cancel-move-btn");
                if (existingMoveBtn) existingMoveBtn.remove();
                if (existingCancelBtn) existingCancelBtn.remove();

                const moveBtn = document.createElement("button");
                moveBtn.className = "question-btn move-task-btn";
                moveBtn.style.background = "#2196f3";
                moveBtn.textContent = isMoveMode ? "Taşımaya Devam Et" : "Taşı";
                moveBtn.onclick = isMoveMode
                    ? () => continueMoveTaskSelection(dayIdx)
                    : () => startMoveTaskSelection(dayIdx);
                buttonGroup.appendChild(moveBtn);

                if (isMoveMode) {
                    const cancelBtn = document.createElement("button");
                    cancelBtn.className = "question-btn cancel-move-btn";
                    cancelBtn.style.background = "#64748b";
                    cancelBtn.textContent = "İptal";
                    cancelBtn.onclick = () => cancelMoveTaskSelection();
            ...
            Array.from(taskList.children).forEach((taskItem, taskIdx) => {
                const existingCheckbox = taskItem.querySelector(".move-task-checkbox");
                if (existingCheckbox) existingCheckbox.remove();
                if (!isMoveMode) return;

                const checkbox = document.createElement("input");
                checkbox.type = "checkbox";
                checkbox.className = "move-task-checkbox";
                checkbox.checked = !!moveTaskSelections[taskIdx];
                checkbox.onchange = (e) => toggleMoveTaskSelection(dayIdx, taskIdx, e.target.checked);
    ...
    function startMoveTaskSelection(dayIdx) {
        const weekKey = getWeekKey(currentWeekStart);
        const dayTasks = scheduleData?.[weekKey]?.[dayIdx]?.tasks || [];
        if (!dayTasks.length) {
            safeShowAlert("Taşınacak görev bulunamadı.");
            return;
        }
        activeMoveSelection = { weekKey, dayIdx };
        moveTaskSelections = {};
        if (typeof renderSchedule === "function") renderSchedule();
        safeShowAlert("Taşımak istediğin görevleri işaretle.");
    }

    function toggleMoveTaskSelection(dayIdx, taskIdx, checked) {
        const currentWeekKey = getWeekKey(currentWeekStart);
        if (!activeMoveSelection
            || activeMoveSelection.weekKey !== currentWeekKey
            || activeMoveSelection.dayIdx !== dayIdx) return;
        moveTaskSelections[taskIdx] = checked;
    }

    function cancelMoveTaskSelection() {
        activeMoveSelection = null;
        moveTaskSelections = {};
        if (typeof renderSchedule === "function") renderSchedule();
    }

    function continueMoveTaskSelection(dayIdx) {
        const weekKey = getWeekKey(currentWeekStart);
        const sourceTasks = scheduleData?.[weekKey]?.[dayIdx]?.tasks || [];
        const selectedIndexes = Object.keys(moveTaskSelections)
            .filter((key) => moveTaskSelections[key])
            .map((key) => parseInt(key, 10))
            .sort((a, b) => a - b);

        if (!selectedIndexes.length) {
            safeShowAlert("Önce taşınacak görevleri seç.");
            return;
        }
        ...
        if (nextDayIdx > 6) {
            ...
            nextWeekKey = getWeekKey(nextWeekDate);
        }

        if (!scheduleData[nextWeekKey]) scheduleData[nextWeekKey] = {};
        if (!scheduleData[nextWeekKey][nextDayIdx]) {
            scheduleData[nextWeekKey][nextDayIdx] = { tasks: [], workedSeconds: 0, questions: 0 };
        }

        const tasksToMove = selectedIndexes
            .map((index) => sourceTasks[index])
            .filter(Boolean)
            .map((task) => ({ ...task }));

        scheduleData[weekKey][dayIdx].tasks = sourceTasks.filter((_, i) => !selectedIndexes.includes(i));
        scheduleData[nextWeekKey][nextDayIdx].tasks.push(...tasksToMove);

        activeMoveSelection = null;
        moveTaskSelections = {};

        if (typeof saveData === "function") saveData({ authorized: true, immediate: true });
        if (typeof renderSchedule === "function") renderSchedule();
        safeShowAlert("Seçilen görevler ertesi güne aktarıldı.");
    }
