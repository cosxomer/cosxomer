(() => {
    ...
    function ensureAdminTimerResetControls() {
        ...
        const supportModalBody = adminSummary.closest('.support-modal-body') || document.querySelector('.support-modal-body');
        if (supportModalBody) {
            supportModalBody.style.gridTemplateColumns = 'minmax(430px, 1.08fr) minmax(0, 1.25fr)';
            supportModalBody.style.alignItems = 'start';
        }

        const composePanel = adminSummary.closest('.support-compose-panel');
        if (composePanel) {
            composePanel.style.minWidth = '430px';
        }
        ...
        if (!panel) {
            ...
            panel.innerHTML = `
                ...
                <div id="support-admin-time-adjust-panel" style="display:flex; flex-direction:column; gap:12px; padding:16px; border-radius:18px; background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.08); width:min(100%, 760px);">
                    ...
                    <div id="support-admin-time-user-results" style="display:flex; flex-direction:column; gap:10px; max-height:420px; overflow:auto; padding-right:4px;"></div>
    ...
    function setAdminTimeAdjustmentSelection(userChoice = null) {
        ...
        if (controls.selectedMeta) {
            ...
            controls.selectedMeta.innerHTML = `
                <div style="font-weight:700; margin-bottom:6px; line-height:1.4; white-space:normal; overflow-wrap:anywhere;">Secilen kullanici: ${escapeAdminTimeAdjustmentHtml(userChoice.primaryLabel)}</div>
                <div style="opacity:0.82; line-height:1.45; white-space:normal; overflow-wrap:anywhere;">${escapeAdminTimeAdjustmentHtml(userChoice.secondaryLabel || userChoice.uid)}</div>
            `;
        }
    }
    ...
    async function renderAdminTimeAdjustmentUserMatches(filterValue = '') {
        ...
        try {
            ...
            controls.results.innerHTML = matches.map(user => {
                ...
                return `
                    <button type="button" data-admin-time-user-uid="${escapeAdminTimeAdjustmentHtml(user.uid)}" style="display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:center; gap:12px; width:100%; min-height:58px; padding:14px 16px; border-radius:14px; border:1px solid ${isSelected ? 'rgba(59,130,246,0.65)' : 'rgba(255,255,255,0.08)'}; background:${isSelected ? 'rgba(37,99,235,0.16)' : 'rgba(15,23,42,0.44)'}; color:var(--header-text); cursor:pointer; text-align:left; box-sizing:border-box; overflow:visible;">
                        <span style="display:flex; flex-direction:column; justify-content:center; gap:6px; min-width:0; min-height:28px; padding-bottom:2px;">
                            <span style="font-weight:700; line-height:1.55; white-space:normal; overflow-wrap:anywhere; display:block;">${escapeAdminTimeAdjustmentHtml(user.primaryLabel)}</span>
                            <span style="font-size:0.84rem; opacity:0.78; line-height:1.4; white-space:normal; overflow-wrap:anywhere;">${escapeAdminTimeAdjustmentHtml(user.secondaryLabel || user.uid)}</span>
                        </span>
                        <span style="flex-shrink:0; padding:5px 9px; border-radius:999px; background:${isSelected ? 'rgba(59,130,246,0.18)' : 'rgba(255,255,255,0.08)'}; font-size:0.74rem; font-weight:700; opacity:0.92; align-self:center;">${isSelected ? 'Secildi' : 'Sec'}</span>
                    </button>
                `;
            }).join('');
